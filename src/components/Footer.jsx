    ...
    <footer className="footer">
      <div className="container footer-inner">
        ...
        <div className="footer-links">
          {links.map((l) => (
            <a href={l.href} key={l.label} className="footer-link" target={l.href.startsWith('http') ? '_blank' : undefined} rel="noreferrer" aria-label={l.label}>
              {l.icon}
            </a>
          ))}
        </div>
        <p className="footer-note">© {new Date().getFullYear()} Salim Technology. All rights reserved.</p>
        <p className="footer-note">BN-9760415</p>
      </div>
    </footer>
  )
}
